...
static ssize_t calibration_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int retval;
	u16 data = 0x08;
	u16 cmd = CY_OPERATE_MODE;

	/* quit idle mode */
	retval = ttsp_write_block_data(g_ts, CY_REG_BASE, sizeof(u8), &cmd, 0x67, true);
	msleep(30);

	if (retval < 0) {
		return sprintf(buf, "cypress touch quit idle mode failed!!\n");
	} else {
		retval = ttsp_write_block_data(g_ts, 0x01, sizeof(u8), &data, 0x67, true);
		if (retval < 0)
			return sprintf(buf, "cypress touch calibration failed!!\n");
		else
			return sprintf(buf, "cypress touch calibration!!\n");
	}
}
...
static ssize_t version_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	fw_version = g_ts->sysinfo_data.app_verh;
	fw_version <<= 4;
	fw_version |= g_ts->sysinfo_data.app_verl;

	return sprintf(buf, "%s%s-%03X%X-%s%X%X\n", Chip_Vendor, Reseved_Chip_Vendor, fw_version, Reseved_Firmware_Info,
		 ConfigVersion, Reseved_ConfigVersion, Reservedinfo);
}

static DEVICE_ATTR(FirmwareVersion, 0644, version_show, NULL);
static DEVICE_ATTR(calibration, 0644, calibration_show, NULL);
...
static int TOUCH_attr_init(void)
{
	...
	ret = sysfs_create_file(TOUCH, &dev_attr_calibration.attr);
...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *dev)
{
	...
	__set_bit(ABS_MT_PRESSURE, input_device->keybit);
